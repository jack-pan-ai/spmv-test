...
namespace merged
{
    ...
    struct AgentFlexSpmv
    {
        ...
        // Tensor and TensorKey for input vector x
          typedef Tensor<ValueT, 1> TensorInput_vector_T; 
  ...
  typedef ReduceTensorByKeyOp<TensorKeyOutput_scatter_T>                        ReduceBySegmentOp_scatter_T; 
  typedef BlockScan< 
            TensorKeyOutput_scatter_T, 
            BLOCK_THREADS, 
        ...
        // [code generation] wrapper pointers for loading the data
          VectorValueIteratorT vector_ptr; 
        ...
        AgentFlexSpmv(
            ...
            : temp_storage(temp_storage.Alias()),
                wd_row_end_offsets(spmv_params.d_row_end_offsets),
                  vector_ptr(spmv_params.vector_ptr), 
    sx_ptr(spmv_params.sx_ptr), 
...
#pragma unroll
            for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ++ITEM)
            {
                ...
                {
                    // [code generation]
                        VectorValueIteratorT sx_ptr_current = sx_ptr +                     (tile_start_coord.y + nonzero_idx) * 1; 
    TensorInput_sx_T sx(sx_ptr_current); 
    ColumnIndicesIteratorT gather_src_ptr_current =                     gather_src_ptr + tile_start_coord.y + nonzero_idx; 
    TensorInput_vector_T                     gather_src(vector_ptr + *gather_src_ptr_current * 1); 


                    // map
                    TensorOutput_mul_T mul = gather_src * sx;
                    // mul_tensor(gather_src, sx, mul);
                    // mul.values[0] = gather_src.values[0] * sx.values[0];
    temp_storage.smem_scatter.s_tile_value_reducer[nonzero_idx] =                     mul; 
            ...
            search_thread_start_coord( 
                ...
                thread_start_coord); 
            // [code generation]
               reduce<1, BlockScan_scatter_T, TensorOutput_scatter_T,                 ReduceBySegmentOp_scatter_T>( 
                temp_storage.smem_scatter.s_tile_value_reducer,                          ///< [in, code gen] Shared memory array of non-zero values for the merge tile 
